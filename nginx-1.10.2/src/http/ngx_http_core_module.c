...
ngx_http_core_run_phases(ngx_http_request_t *r)
{
    ...
    while (ph[r->phase_handler].checker) {  // why has while, see ngx_http_core_access_phase func

        rc = ph[r->phase_handler].checker(r, &ph[r->phase_handler]); // may call ngx_http_core_access_phase
    ...
    ngx_http_phase_handler_t *ph)
{
    ...
    if (r->content_handler) {     // such as: ngx_http_proxy_handler
...
// meet "server" in nginx will call this func(search ngx_string("server")), will call create_srv_conf and create_loc_conf
// meet "http" will call create_main_conf, create_srv_conf and create_loc_conf, see ngx_http_block func
static char *
