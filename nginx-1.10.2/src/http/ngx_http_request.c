...
ngx_http_wait_request_handler(ngx_event_t *rev)
{
    ...
    n = c->recv(c, b->last, size); // call ngx_unix_recv function
...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_AGAIN) {
            n = ngx_http_read_request_header(r);  // read request line, such as " Get xxx http/1.1 ", the first line is request line , others are request header
        ...
        if (rc == NGX_OK) {
            ...
            if (r->http_version < NGX_HTTP_VERSION_10) {  // in this version, has no request header, we need to call ngx_http_process_request_headers
...
ngx_http_process_request_headers(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_AGAIN) 
		{
            ...
            n = ngx_http_read_request_header(r); // read request header:  such as: Host: www.baidu.com, content-length: 133
        ...
        if (rc == NGX_HTTP_PARSE_HEADER_DONE) {
            ...
            rc = ngx_http_process_request_header(r);  // process request header
...
ngx_http_set_virtual_server(ngx_http_request_t *r, ngx_str_t *host)
{
    ...
    r->srv_conf = cscf->ctx->srv_conf;  // has find the conresponding configure in nginx.conf
