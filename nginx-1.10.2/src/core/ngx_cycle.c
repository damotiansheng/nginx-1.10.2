...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    if (ngx_conf_param(&conf) != NGX_CONF_OK) {    // parse the conf file and set the data
        environ = senv;
        ngx_destroy_cycle_pools(&conf);
        return NULL;
    }

    if (ngx_conf_parse(&conf, &cycle->conf_file) != NGX_CONF_OK) {  // parse the conf file and set the data
        environ = senv;
        ngx_destroy_cycle_pools(&conf);
        return NULL;
    }
...
/*
	ngx_log_error(NGX_LOG_EMERG, log, 0,
                          "old_cycle->listening.nelts= \"%d\"",
                          old_cycle->listening.nelts ); // 0 
	
	ngx_log_error(NGX_LOG_EMERG, log, 0,
                          "cycle->listening.nelts= \"%d\"",
    ...
    if (old_cycle->listening.nelts) 
	{
		...
		ngx_log_error(NGX_LOG_ERR, cycle->log, ngx_errno,
                      "ngx_init_cycle:hyp:" " \"%d\" ", cycle->listening.nelts );

		ngx_log_error(NGX_LOG_ERR, cycle->log, ngx_errno,
                      "ngx_init_cycle:old-hyp:" " \"%d\" ", old_cycle->listening.nelts );
		*/
...
// after call ngx_init_zone_pool, we can allocate and free in shared memory by the means of slab
static ngx_int_t
...
// reopen files in cycle->open_files
void
