...
ngx_clone_listening(ngx_conf_t *cf, ngx_listening_t *ls)
{
    ...
    for (n = 1; n < ccf->worker_processes; n++) 
	{
        ...
        *ls = ols;                  // see ngx_event_process_init func
        ls->worker = n;
    }
...
// set some options for cycle->listening
ngx_int_t
ngx_set_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) 
	{
		...
		// get ip addr saved in ls[i].addr_text.data
...
// bing and listen for cycle->listening
ngx_int_t
...
// set some options for cycle->listening sockets
void
...
// get a free connection from ngx_cycle->free_connections
ngx_connection_t *
ngx_get_connection(ngx_socket_t s, ngx_log_t *log)
{
    ...
    ngx_cycle->free_connections = c->data;  // pointer to the next node
...
// http://blog.csdn.net/brainkick/article/details/7321894
// http://www.xuebuyuan.com/1722409.html
static void
ngx_drain_connections(void)
{
    ...
    for (i = 0; i < 32; i++)  // to release some long connection to free_connections
	{
        ...
        c->close = 1;
        c->read->handler(c->read); // will call ngx_http_keepalive_handler to release this connection
    }
}
