...
// ngx_core_module_ctx is saved in ctx of ngx_module_t
...
ngx_exec_new_binary(ngx_cycle_t *cycle, char *const *argv)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        p = ngx_sprintf(p, "%ud;", ls[i].fd);  // ls[0].fd=6, ls[1].fd=7
    }

    *p = '\0';  // "NGINX=6;7;"

    env[n++] = var; // env[0]="NGINX=6;7;"   // new master process will call ngx_add_inherited_sockets func to get those listen fd
	...
	*/
	...
	// ccf->oldpid.data="/usr/local/nginx/logs/nginx.pid.oldbin", is initialized in ngx_core_module_init_conf func
	// ccf->pid.data="/usr/local/nginx/logs/nginx.pid"
    ...
    pid = ngx_execute(cycle, &ctx);  // pid is the pid of new master process
...
// save paramter info from argv saved in global variable
static ngx_int_t
...
// save argv,argc, environ to global variable:ngx_argv, ngx_argc and ngx_os_environ
static ngx_int_t
...
// save prefix, conf_prefix, conf_file, conf_param in cycle
static ngx_int_t
...
// to create ngx_core_conf_t and return it which saved in cycle->conf_ctx
static void *
