...
ngx_event_module_t  ngx_epoll_module_ctx = {
    &epoll_name,
    ngx_epoll_create_conf,               /* create configuration */  // will be called in ngx_events_block 
    ngx_epoll_init_conf,                 /* init configuration */  // will be called in ngx_events_block 
...
ngx_epoll_notify_init(ngx_log_t *log)
{
    ...
    // set notify_event.data in ngx_epoll_notify func
...
ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    if (e->active)  // is active and we should save pre
	{
        op = EPOLL_CTL_MOD;
        events |= prev;
...
ngx_epoll_del_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    if (flags & NGX_CLOSE_EVENT)  // see ngx_close_listening_sockets function
	{
        ev->active = 0;
        return NGX_OK;
    }
...
// enable c->fd for read and write
static ngx_int_t
...
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    if (events == 0) 
	{
        if (timer != NGX_TIMER_INFINITE) //timeout
		{
            return NGX_OK;
        }
    ...
    for (i = 0; i < events; i++) 
	{
        ...
        if (c->fd == -1 || rev->instance != instance)  // is expired event or not
		{
